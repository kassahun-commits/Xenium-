#!/usr/bin/env python3
"""
Multi-page per-mouse mean BAR PLOTS for ASTROCYTES across all 7 condition
groups. Single cell type (no further sub-classification).

Gene list = same custom list used for the neuron version + cell-type
identity markers + MCT family + GFP + alcohol-metabolism enzymes
(Adh5, Aldh2, Cyp2e1, Cat, Acss1/2, Acly, Aldh1a2, Eno1/2, Ldha/b) +
astrocyte-specific markers (Slc6a11, Gja1, S100b, Aldoc) +
oxidative-stress / detox (Sod1, Sod2, Nfe2l2, Hmox1).

Output: multi-page PDF (20 genes per page).
"""
...
# User-provided custom gene list (same as for neurons)
USER_GENES = [
...
# Astrocyte-relevant gene set, ordered for the figure:
GENE_LIST = (
    # Cell-type identity (verify the cells we're looking at)
    ['Gfap', 'Aqp4', 'Slc1a3', 'Aldh1l1', 'S100b', 'Aldoc', 'Gja1',
     'Slc1a1', 'Slc6a11',
     # Neuronal markers (for contrast / cross-check)
     'Rbfox3', 'Snap25', 'Syn1',
     # MCT family
     'Slc16a1', 'Slc16a7', 'Slc16a3',
     # Reporter
     'GFP',
     # Alcohol-metabolizing enzymes (NEW)
     'Adh1', 'Adh5', 'Aldh2', 'Aldh1a2', 'Cyp2e1', 'Cat',
     # Acetate / Ac-CoA + glycolysis / TCA related to ethanol metabolism
     'Acss1', 'Acss2', 'Acly',
     'Eno1', 'Eno2', 'Ldha', 'Ldhb',
     # Oxidative stress / detox
     'Sod1', 'Sod2', 'Gpx1', 'Gpx4', 'Nfe2l2', 'Hmox1',
    ]
...
def plot_barpage(genes_page, sample_df, page_idx, n_pages, label, out_pdf_page):
    ...
    fig.suptitle(f'Astrocytes — per-mouse mean bar plots — '
...
def main():
    ...
    log_path = args.out_dir / f'{args.label}_Astrocyte_BarPlots_log.txt'
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s | %(levelname)s | %(message)s',
                        handlers=[logging.FileHandler(log_path, mode='w'),
                                  logging.StreamHandler(sys.stdout)])
    logging.info('=== Astrocyte multi-page bar plots ===')
    ...
    sub = adata[adata.obs['celltype'].astype(str) == 'Astrocyte'].copy()
    ...
    logging.info('Astrocytes after filter: %d cells', sub.shape[0])
    ...
    s_counts.to_csv(args.out_dir / f'{args.label}_Astrocyte_AllGroups_cellcounts_{args.date}.csv', index=False)

    available = list(dict.fromkeys(g for g in GENE_LIST if g in sub.var_names))
    ...
    sample_csv = args.out_dir / f'{args.label}_Astrocyte_AllGroups_persample_means_{args.date}.csv'
    sample_df.to_csv(sample_csv, index=False)
    logging.info('Wrote %s', sample_csv)

    per_page = args.per_page
    n_pages = (len(available) + per_page - 1) // per_page
    pdf_out = args.out_dir / f'{args.label}_Astrocyte_AllGroups_BarPlots_{args.date}.pdf'
    with PdfPages(pdf_out) as pdf:
        for page_idx in range(n_pages):
            chunk = available[page_idx * per_page:(page_idx + 1) * per_page]
            plot_barpage(chunk, sample_df, page_idx, n_pages, args.label, pdf)
