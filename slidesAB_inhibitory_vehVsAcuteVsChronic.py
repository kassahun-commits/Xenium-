#!/usr/bin/env python3
"""
Inhibitory neurons only: H2O_veh vs EtOH_veh (acute) vs ChronicEtOH.

Same pipeline + plotting as slidesAB_excitatory_vehVsAcuteVsChronic.py
but inhibitory-neuron sub-classification (GABAergic markers).

Inhibitory markers: Gad1, Gad2, Slc32a1 (VGAT), Pvalb, Sst, Vip, Reln, Lhx6.

Force-includes MCT family (MCT1/2/4). Gene labels use 'gene (alias)' format.
"""
...
def main():
    ...
    log_path = args.out_dir / f'{args.label}_InhibNeuron_vehAcuteChronic_log.txt'
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s | %(levelname)s | %(message)s',
                        handlers=[logging.FileHandler(log_path, mode='w'),
                                  logging.StreamHandler(sys.stdout)])
    logging.info('=== Inhibitory neurons: H2O_veh vs EtOH_veh vs ChronicEtOH ===')
    ...
    logging.info('Inhibitory markers in panel: %s', inhib_present)
    ...
    inhib = adata[is_inhib].copy()
    logging.info('Inhibitory neurons total: %d', inhib.shape[0])

    sub = inhib[inhib.obs['group'].astype(str).isin(GROUPS)].copy()
    ...
    counts = (sub.obs.groupby(['group', 'sample_id'], observed=True)
              .size().reset_index(name='n_inhib'))
    counts.to_csv(args.out_dir / f'{args.label}_Inhib_vehAcuteChronic_cellcounts_{args.date}.csv',
                  index=False)
    logging.info('Per-sample inhibitory counts:\n%s', counts.to_string(index=False))
    ...
    if min(n_per_group) < 50:
        logging.warning('Very few inhibitory cells in at least one group — DE may be noisy.')
    ...
    for test in ['EtOH_veh', 'EtOH_MCT1i', 'ChronicEtOH']:
        ...
        if (d2.obs['de_group'].value_counts() < 20).any():
            logging.warning('Skipping %s vs H2O_veh: insufficient cells', test)
            continue
    ...
    de_long.to_csv(args.out_dir / f'{args.label}_Inhib_vehAcuteChronic_DE_{args.date}.csv', index=False)
    ...
    logging.info('Top UP: %s', top_up.index.tolist())
    logging.info('Top DOWN: %s', top_dn.index.tolist())
    ...
    for i, gene in enumerate(top_genes):
        ...
        y_max = 0.0
        positions = {g: idx for idx, g in enumerate(GROUPS)}
        for grp in GROUPS:
            dg = d[d['group'] == grp]
    ...
    fig.suptitle(
        f'{args.label}: Inhibitory neurons — H2O_veh vs EtOH_veh vs EtOH_MCT1i vs ChronicEtOH\n'
    ...
    pdf_out = args.out_dir / f'{args.label}_Inhib_vehAcuteChronic_BoxPlots_{args.date}.pdf'
