#!/usr/bin/env python3
"""
Master PI PDF — V1 + V2 — Excit | Inhib | Astrocyte — LFC 0.5, xlim ±5.

3-cell-type version:
  * Excitatory: Neuron with Slc17a7/Camk2a/Camk2b/Satb2/Tbr1/Neurod6 score >0
  * Inhibitory: Neuron with Gad1/Gad2/Slc32a1/Pvalb/Sst/Vip/Reln/Lhx6 score >0
  * Astrocyte: top-scoring lineage = Astrocyte
  * LFC threshold = 0.5; volcano x-axis clipped to ±5.

Layout per contrast (per V1 / V2 section):
  - 1 volcano page (3 cell-type panels: Excit | Inhib | Astrocyte)
  - 2 top-hits pages (page 1 = top 25 UP, page 2 = top 25 DOWN) in 5x5 grid
  - Each top-hit panel has 21 bars (7 condition clusters × 3 cell types)
"""
...
def cell_type_data(adata):
    ...
    # Additional Excit / Inhib subtype scoring (within Neuron)
    ...
    sc.tl.score_genes(adata, gene_list=inhib_present, score_name='score_Inhibitory',
                      random_state=0, n_bins=25)
    # Assign refined subtype: Excitatory / Inhibitory / Astrocyte (or Other)
    ...
    subtype[is_excit] = 'Excitatory'  # excit takes precedence if both score
    subtype[is_astro] = 'Astrocyte'
    adata.obs['subtype'] = pd.Categorical(subtype,
        categories=['Excitatory', 'Inhibitory', 'Astrocyte', 'Other'])
    return adata
...
def run_de_2subtypes(adata):
    """Run DE for all comparisons in Excit / Inhib / Astrocyte subsets."""
    rows = []
    cache = {}  # (subtype, test, ref) -> (df, meta)
    for subtype in SUBTYPES:
        sub_all = adata[adata.obs['subtype'].astype(str) == subtype].copy()
...
def compute_persample_means(adata, gene_list):
    persample = {}
    for subtype in SUBTYPES:
        sub = adata[(adata.obs['subtype'].astype(str) == subtype) &
...
def plot_compact_panel(ax, gene, persample_by_subtype, direction_tag=None):
    """Grouped-cell-type bar panel for Excit + Inhib + Astro (21 bars per panel)."""
...
def main():
    ...
    logging.info('=== PI master (3 cell types; LFC %g; xlim ±5) ===', LFC_THRESH)
    ...
    # ---- Build PDFs ----
    tmp = args.out.parent / '_tmp_master_3CT'
    ...
    cover_body = (
        ...
        f"Cell types: Excitatory neuron / Inhibitory neuron / Astrocyte.\n"
        f"Significance thresholds: |log2 FC| >= {LFC_THRESH:g} AND adj p < {PADJ_THRESH:g}.\n"
        "Volcano x-axis clipped to ±5 (extreme outliers omitted for readability).\n\n"
        "Structure:\n"
        "  V1 (broad-ROI / full punches): volcano grid (3 panels per contrast)\n"
        ...
        "cluster, three bars side-by-side colored by cell type:\n"
        ...
        "the log2FC magnitude, e.g. [E:↑(+0.8) A:↓(-0.6)]\n"
        "  E = Excitatory, I = Inhibitory, A = Astrocyte.\n\n"
    ...
    make_cover('Xenium May 2026 — FINAL PI summary  (3 cell types; LFC 0.5; xlim ±5)',
    ...
    v1_de.to_csv(args.out.parent / f'PI_Master_3CT_V1_DE_{date.today().isoformat()}.csv', index=False)
    v2_de.to_csv(args.out.parent / f'PI_Master_3CT_V2_DE_{date.today().isoformat()}.csv', index=False)
