"""
Proportional Venn diagram — Chromatin AW Downregulated proteins (NO LABELS)
==========================================================================
Labels-off twin of make_venn_chromatin_AW_down_proportional.py:
identical circles, colors, and region counts, but no set labels,
no title, and no footnote (for figure assembly in Illustrator).

Keyed on Accession (unique per protein) so AW = 642; three AW-down
proteins share the symbol "Uncharacterized" and must not collapse.

Output: Venn_Chromatin_AW_Down_Proportional_nolabels.pdf / .png
"""
...
OUT_PDF = os.path.join(SCRIPT_DIR, 'Venn_Chromatin_AW_Down_Proportional_nolabels.pdf')
OUT_PNG = os.path.join(SCRIPT_DIR, 'Venn_Chromatin_AW_Down_Proportional_nolabels.png')
...
def get_sig_genes(naive_cols, cond_cols, direction):
    ...
    # Key on Accession (unique per protein); see module docstring.
    return set(df.loc[mask, 'Accession'].tolist())

AW    = get_sig_genes(n_cols, aw_cols, 'down')
INTOX = get_sig_genes(n_cols, i_cols,  'down')
PA    = get_sig_genes(n_cols, pa_cols, 'down')
...
print(f'AW down:    n={len(AW)}')
print(f'Intox down: n={len(INTOX)}')
print(f'PA down:    n={len(PA)}')
...
# No set labels (labels-off version)
v = venn3(subsets=layout, set_labels=('', '', ''), ax=ax)
...
# Ensure any set labels are blank
for lbl in v.set_labels:
    if lbl:
        lbl.set_text('')

# No title, no footnote.
plt.tight_layout()
