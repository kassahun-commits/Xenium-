"""
Proportional Venn diagram — Chromatin AW Downregulated proteins
===============================================================
AW Decreased vs Intox Decreased vs PA Decreased (Chromatin fraction).
Circle areas proportional to actual set sizes.
AW-M-3 excluded. Chromatin: Keep+Review only.

Mirrors make_venn_chromatin_AW_up_proportional.py exactly; only the
direction (down) and the labels/title/output names differ.

Output: Venn_Chromatin_AW_Down_Proportional.pdf / .png
"""
...
OUT_PDF = os.path.join(SCRIPT_DIR, 'Venn_Chromatin_AW_Down_Proportional.pdf')
OUT_PNG = os.path.join(SCRIPT_DIR, 'Venn_Chromatin_AW_Down_Proportional.png')
...
def get_sig_genes(naive_cols, cond_cols, direction):
    ...
    # Key on Accession (unique per protein) rather than Gene symbol: 3 distinct
    # AW-down proteins share the symbol "Uncharacterized" and would otherwise
    # collapse to one, undercounting and risking false cross-condition overlaps.
...
ax.set_title('Chromatin — AW Downregulated Proteins\nOverlap Across Conditions',
