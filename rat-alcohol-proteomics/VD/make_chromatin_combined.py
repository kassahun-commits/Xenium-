"""
Chromatin AW combined figure:
  Top:    Single VP showing both red (increased) and blue (decreased),
          jitter ±0.20 applied to fix the stripe visually
  Bottom: Venn increased (left) + Venn decreased (right)

AW-M-3 excluded. Chromatin: Keep+Review filter.
Output: Chromatin_AW_Combined.pdf
"""
...
print(f'Chromatin AW:  ↑{len(aw_up)}  ↓{len(aw_dn)}  (AW-M-3 excluded)')
...
# ── Volcano panels — identical to individual VP style, with jitter ─────────────
C_OTHER = '#C8C8C8'
...
def style_vp_ax(ax, hi, lo, c_hi, title, direction, n_hi):
    ax.scatter(fx[ns], cr[ns], s=10, color=C_NS,    alpha=0.40, linewidths=0, rasterized=True)
    ax.scatter(fx[lo], cr[lo], s=18, color=C_OTHER, alpha=0.45, linewidths=0, rasterized=True)
    ax.scatter(fx[hi], cr[hi], s=35, color=c_hi,    alpha=0.90, linewidths=0, rasterized=True)
...
style_vp_ax(ax_vp_up, up, down, C_UP,   'Chromatin AW vs Naïve — Increased', 'up',   n_up)
style_vp_ax(ax_vp_dn, down, up, C_DOWN, 'Chromatin AW vs Naïve — Decreased', 'down', n_down)
