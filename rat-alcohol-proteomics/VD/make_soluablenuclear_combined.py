"""
Soluble Nuclear AW combined figure  (mirrors make_chromatin_combined.py)
========================================================================
...
AW-M-3 excluded (already absent from this file).
Soluble Nuclear: Keep + Review filter applied.
Output: SolubleNuclear_AW_Combined.pdf
"""
...
OUT  = 'SolubleNuclear_AW_Combined.pdf'
...
# AW-M-3 already excluded in this file
...
# ── Load data ──────────────────────────────────────────────────────────────────
raw = pd.read_excel(FILE, sheet_name='Soluble nuclear')
df  = raw.reset_index(drop=True)
print(f'Soluble Nuclear (all proteins, no filter): {len(df)} proteins')
...
print(f'Soluble Nuclear AW:  ↑{len(aw_up)}  ↓{len(aw_dn)}  (AW-M-3 excluded)')
print(f'Soluble Nuclear Intox: ↑{len(i_up)}  ↓{len(i_dn)}')
print(f'Soluble Nuclear PA:    ↑{len(pa_up)}  ↓{len(pa_dn)}')
...
style_vp_ax(ax_vp_up, up,   down, C_UP,   'Soluble Nuclear AW vs Naïve — Increased', 'up',   n_up)
style_vp_ax(ax_vp_dn, down, up,   C_DOWN, 'Soluble Nuclear AW vs Naïve — Decreased', 'down', n_down)
