"""
Translocation TRUE Union Heatmap — p < 0.10 threshold
======================================================
Same as make_translocation_true_union_heatmap.py but using BH FDR p_adj < 0.10.

True translocation criteria (per condition):
  Into Chromatin:  Sig=True  AND  Direction=Into_Chromatin  AND  SN_FC  < -0.5
  Into SN:         Sig=True  AND  Direction=Into_SN  AND  SN_FC  > 0  AND  Chrom_FC < -0.5

Excel output has Metascape-ready gene lists per condition × direction.

Outputs:
  Translocation_TrueUnion_stats_p10.xlsx
  Translocation_TrueUnion_heatmap_p10.pdf / .png
"""
...
OUT_XLSX = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_stats_p10.xlsx')
OUT_PDF  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_heatmap_p10.pdf')
OUT_PNG  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_heatmap_p10.png')
...
n_true_ch = true_ch_any[true_any].sum()
n_true_sn = true_sn_any[true_any].sum()
print(f'\nTrue union: {len(df_true)} proteins  (Into_CH in ≥1: {n_true_ch}  |  Into_SN in ≥1: {n_true_sn})')

# ── Save Excel (Metascape-ready) ──────────────────────────────────────────────
# Annotate df_true with True_Direction_<cond> for each condition
for cond_name, _, _ in CONDITIONS:
    sig   = df_all.loc[df_true.index.map(lambda i: df_all.index[df_all['Gene'] == df_true.loc[i, 'Gene']].item()) if False else df_true.index, :]
    # Simpler: recompute per true protein
    pass

# Add per-condition true direction column
for cond_name, _, _ in CONDITIONS:
    sn_fc  = pd.to_numeric(df_true[f'SN_FC_{cond_name}'],    errors='coerce')
    ch_fc  = pd.to_numeric(df_true[f'Chrom_FC_{cond_name}'], errors='coerce')
    sig_c  = df_true[f'Sig_{cond_name}']
    dirn_c = df_true[f'Direction_{cond_name}']
    true_ch = sig_c & (dirn_c == 'Into_Chromatin') & (sn_fc < -FC_THRESH)
    true_sn = sig_c & (dirn_c == 'Into_SN')        & (sn_fc >  0)          & (ch_fc < -FC_THRESH)
    df_true[f'TrueDirection_{cond_name}'] = np.where(
        true_ch, 'Into_Chromatin',
        np.where(true_sn, 'Into_SN', 'NS'))

# Metascape gene lists
meta_rows = []
for cond_name, _, _ in CONDITIONS:
    for dirn in ['Into_Chromatin', 'Into_SN']:
        mask = df_true[f'TrueDirection_{cond_name}'] == dirn
        for g in df_true.loc[mask, 'Gene'].tolist():
            meta_rows.append({'List': f'{cond_name}_True_{dirn}',
                               'Condition': cond_name, 'Direction': dirn, 'Gene': g})
df_meta = pd.DataFrame(meta_rows)

with pd.ExcelWriter(OUT_XLSX, engine='openpyxl') as writer:
    df_true.to_excel(writer,  sheet_name='True_Union',      index=False)
    df_all.to_excel(writer,   sheet_name='All_proteins',    index=False)
    df_meta.to_excel(writer,  sheet_name='Metascape_lists', index=False)
    for cond_name, _, _ in CONDITIONS:
        for dirn, short in [('Into_Chromatin','TrueCH'), ('Into_SN','TrueSN')]:
            mask = df_true[f'TrueDirection_{cond_name}'] == dirn
            sub = df_true.loc[mask, ['Gene', f'Chrom_FC_{cond_name}', f'SN_FC_{cond_name}',
                                      f'Interaction_{cond_name}', f'p_adj_{cond_name}',
                                      f'TrueDirection_{cond_name}']].copy()
...
fig.text(0.5, (FIG_H-TOP_PAD*0.35)/FIG_H,
         f'True Protein Translocation — Union Across All Conditions  (n = {n_prot})  [p < 0.10]',
...
fig.text(0.5, 0.008,
         ...
         f'Union = true in ≥1 condition  |  Ward clustering  |  AW-M-3 excluded',
