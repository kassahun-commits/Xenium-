...
OUT_PDF  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_byCondition_heatmap_p10_fc1_minFC1.pdf')
OUT_PNG  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_byCondition_heatmap_p10_fc1_minFC1.png')
...
print(f'\nTrue union (before FC filter): {len(df_true)} proteins')
...
max_abs_fc = df_true[fc_cols].abs().max(axis=1)
df_true    = df_true[max_abs_fc >= 1.0].copy().reset_index(drop=True)
print(f'True union (after |FC| ≥ 1 filter): {len(df_true)} proteins')
...
fig.text(0.5, (FIG_H-TOP_PAD*0.35)/FIG_H,
         f'True Protein Translocation — Chromatin & SN by Condition  (n = {n_prot})  [p < 0.10 | |FC| ≥ 1]',
