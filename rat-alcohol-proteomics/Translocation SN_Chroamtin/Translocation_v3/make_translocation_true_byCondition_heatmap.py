"""
Translocation TRUE Union Heatmap — by-condition layout
=======================================================
Same 207-protein true-translocation union as make_translocation_true_union_heatmap.py.
Same delta-delta test, same Ward clustering on translocation index.

Layout change: instead of two stacked fraction bands (Chrom / SN each with 3 condition rows),
this version uses THREE condition bands (Intox / AW / PA), each showing Chromatin and
Soluble Nuclear as adjacent rows so the divergence between fractions is immediately visible.

Annotation strip (top): Into_CH / Into_SN / NS per condition, one row per condition.

Outputs:
  Translocation_TrueUnion_byCondition_heatmap.pdf / .png
  (reuses the same Translocation_TrueUnion_stats.xlsx)
"""
...
OUT_PDF  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_byCondition_heatmap.pdf')
OUT_PNG  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_byCondition_heatmap.png')
...
for cond_name, _, _ in CONDITIONS:
    ...
    true_ch = sig & (dirn == 'Into_CH') & (sn_fc  < -FC_THRESH)
    true_sn = sig & (dirn == 'Into_SN') & (sn_fc  >  0)        & (ch_fc < -FC_THRESH)
    true_ch_any = true_ch_any | true_ch
    true_sn_any = true_sn_any | true_sn
    print(f'{cond_name}: True Into_CH={true_ch.sum()}  True Into_SN={true_sn.sum()}')

true_any  = true_ch_any | true_sn_any
df_true   = df_all[true_any].copy().reset_index(drop=True)
print(f'\nTrue union: {len(df_true)} proteins')

# ── Hierarchical clustering (same as original) ────────────────────────────────
ti_cols = [f'Interaction_{c}' for c, _, _ in CONDITIONS]
ti_mat  = df_true[ti_cols].fillna(0).values
Z       = linkage(ti_mat, method='ward', metric='euclidean')
order   = leaves_list(Z)
df_plot = df_true.iloc[order].reset_index(drop=True)
n_prot  = len(df_plot)
print(f'Clustering done. n = {n_prot}')
...
# ── Layout ────────────────────────────────────────────────────────────────────
# Each condition band = 2 fraction rows (Chromatin + SN)
FRAC_ROW_H   = 0.50    # height per fraction row (inches)
BAND_H       = 2 * FRAC_ROW_H   # 1.0 inch per condition band
ANNOT_ROW_H  = 0.13    # annotation strip row height per condition
ANNOT_H      = 3 * ANNOT_ROW_H  # total annotation strip
ANNOT_GAP    = 0.18    # gap between annotation strip and first condition band
SECTION_GAP  = 0.28    # gap between condition bands
N_COND       = 3

LEFT_IN      = 2.60    # left margin (labels)
RIGHT_IN     = 2.80    # right margin (labels + colorbar)
TOP_PAD      = 0.55
BOT_PAD      = 1.20
...
FIG_H = (TOP_PAD
         + ANNOT_H + ANNOT_GAP
         + N_COND * BAND_H + (N_COND - 1) * SECTION_GAP
         + BOT_PAD)

fig      = plt.figure(figsize=(FIG_W, FIG_H), facecolor='white')
y_cursor = FIG_H - TOP_PAD   # inches from bottom, counting down
...
# Row labels for annotation strip (right side)
...
# ── Three condition bands, each with Chrom + SN rows ─────────────────────────
...
for ci, (cond_name, _, _) in enumerate(CONDITIONS):
    ...
    # Thin divider line between fractions
    ax_ch.axhline(0.5, color='white', linewidth=1.0, zorder=3)

    # Row label: Chromatin (right side)
    ...
    # --- SN row (bottom row of this band) ---
    ...
    ax_sn.imshow(sn_mat, aspect='auto', cmap=CMAP, norm=norm,
                 interpolation='nearest')
    ax_sn.set_xticks([]); ax_sn.set_yticks([])
    for sp in ax_sn.spines.values(): sp.set_visible(False)

    # Row label: SN (right side)
    fig.text((LEFT_IN + hm_w + 0.10) / FIG_W,
             (y_band_bot + FRAC_ROW_H / 2) / FIG_H,
             'Soluble\nNuclear', ha='left', va='center',
             fontsize=7.5, fontfamily='Arial', color='#111111')

    # Band bracket / condition name on the left
    ...
    # Thin separator line between the two fraction rows
    # (drawn in figure coords as a horizontal line between the two axes)
    sep_y = (y_band_bot + FRAC_ROW_H) / FIG_H
    fig.add_artist(plt.Line2D(
        [LEFT_IN / FIG_W, (LEFT_IN + hm_w) / FIG_W],
        [sep_y, sep_y],
...
fig.text(0.5, (FIG_H - TOP_PAD * 0.35) / FIG_H,
         f'True Protein Translocation — Chromatin & SN by Condition  (n = {n_prot})',
         ha='center', va='top',
         fontsize=10, fontweight='bold', fontfamily='Arial', color='#1A3A5C')

fig.text(0.5, 0.008,
         f'True Into_CH: Sig + Direction=Into_CH + SN_FC < -0.5  |  '
         f'True Into_SN: Sig + Direction=Into_SN + SN_FC > 0 + Chrom_FC < -0.5  |  '
         f'Union = true in ≥1 condition  |  Hierarchical clustering (Ward)  |  AW-M-3 excluded',
