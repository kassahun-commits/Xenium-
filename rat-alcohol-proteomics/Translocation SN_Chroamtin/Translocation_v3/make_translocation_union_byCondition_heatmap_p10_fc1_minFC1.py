"""
Translocation Union Heatmap — by-condition layout, p < 0.10
============================================================
All translocation-significant proteins (union, sig in ≥1 condition), p_adj < 0.10.
Layout: 3 condition bands (Intox / AW / PA), each showing Chromatin and SN
as adjacent rows so divergence between fractions is visible per condition.

Outputs:
  Translocation_Union_byCondition_heatmap_p10.pdf / .png
  (stats saved in Translocation_Union_stats_p10.xlsx by the companion script)
"""
...
OUT_PDF  = os.path.join(SCRIPT_DIR, 'Translocation_Union_byCondition_heatmap_p10_fc1_minFC1.pdf')
OUT_PNG  = os.path.join(SCRIPT_DIR, 'Translocation_Union_byCondition_heatmap_p10_fc1_minFC1.png')
...
print(f'Union proteins (before FC filter): {len(df_union)}')

# ── FC filter: keep only proteins with |FC| ≥ 1 in at least one condition/fraction
fc_cols = [f'Chrom_FC_{c}' for c, _, _ in CONDITIONS] + \
          [f'SN_FC_{c}'    for c, _, _ in CONDITIONS]
max_abs_fc = df_union[fc_cols].abs().max(axis=1)
df_union   = df_union[max_abs_fc >= 1.0].copy().reset_index(drop=True)
print(f'Union proteins (after |FC| ≥ 1 filter): {len(df_union)}')
...
# Dynamic VMAX: set from actual max |FC| in the filtered dataset
fc_cols_all  = [f'Chrom_FC_{c}' for c, _, _ in CONDITIONS] + \
               [f'SN_FC_{c}'    for c, _, _ in CONDITIONS]
data_max_abs = float(df_plot[fc_cols_all].abs().max().max())
VMAX         = max(float(np.ceil(data_max_abs)), 1.0)
print(f'Dynamic VMAX = {VMAX:.2f}  (data max |FC| = {data_max_abs:.3f})')
...
for ci, (cond_name, _, _) in enumerate(CONDITIONS):
    ...
    # Separator line between fractions
...
tick_step = 1 if VMAX <= 5 else 2
cb_ticks   = list(range(int(-VMAX), int(VMAX)+1, tick_step))
cb.set_ticks(cb_ticks); cb.ax.tick_params(labelsize=7)
...
fig.text(0.5, (FIG_H-TOP_PAD*0.35)/FIG_H,
         f'Protein Translocation — Chromatin & SN by Condition  (n = {n_prot})  [p < 0.10 | |FC| ≥ 1]',
         ha='center', va='top', fontsize=10, fontweight='bold',
         fontfamily='Arial', color='#1A3A5C')
fig.text(0.5, 0.008,
         'Union: BH FDR p_adj < 0.10 in ≥1 condition  |  '
