"""
Translocation TRUE Union Heatmap
=================================
Same delta-delta interaction test as make_translocation_union_heatmap.py,
but filtered to proteins showing TRUE translocation in at least one condition.

True translocation criteria (per condition):
  Into Chromatin:  Sig=True  AND  Direction=Into_CH  AND  SN_FC  < -0.5
  Into SN:         Sig=True  AND  Direction=Into_SN  AND  SN_FC  >  0
                                                      AND  Chrom_FC < -0.5

Proteins that pass in ≥1 condition are included.
Hierarchical clustering (Ward) on translocation index across 3 conditions.

Outputs:
  Translocation_TrueUnion_stats.xlsx
  Translocation_TrueUnion_heatmap.pdf / .png
"""
...
OUT_XLSX = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_stats.xlsx')
OUT_PDF  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_heatmap.pdf')
OUT_PNG  = os.path.join(SCRIPT_DIR, 'Translocation_TrueUnion_heatmap.png')

FC_THRESH = 0.5   # for "true" translocation filter
...
# ── Layout (same constants as union heatmap) ──────────────────────────────────
...
FIG_H = (TOP_PAD + ANNOT_H + ANNOT_GAP
         + BAND_H + SECTION_GAP + BAND_H + BOT_PAD)
...
fig.text(0.5, (FIG_H - TOP_PAD * 0.35) / FIG_H,
         f'True Protein Translocation — Union Across All Conditions  (n = {n_prot})',
         ha='center', va='top',
