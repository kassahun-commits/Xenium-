...
OUT  = 'Heatmap_Nuc_Chrom_AW.pdf'

CORR_THRESH = 3.3
FC_THRESH   = 0.5
ROW_H_IN    = 0.013   # inches per protein row

# Yellow-white-blue colormap
...
# Condition groups with pastel header colors and replicate suffixes
GROUP_META = [
...
SHEETS = [
    ('Chromatin',       'Chromatin',       'Chrom_'),
    ('Soluble nuclear', 'Soluble nuclear', 'Nuc_'),
]
...
compartment_data = []
for label, excel_name, prefix in SHEETS:
    ...
    # Union: significant in at least one non-naive condition
...
# ── Figure layout: 2 compartments side by side, heights proportional ──
...
fig_w      = N_COMP * comp_w + (N_COMP - 1) * GAP + 1.2   # +cbar margin
...
for ci, d in enumerate(compartment_data):
    ...
    for gname, gcol, gcols in groups:
        ...
        ax_header.add_patch(mpatches.FancyBboxPatch(
            ...
            boxstyle='round,pad=0.02', facecolor=gcol,
    ...
    # Colorbar after last compartment
    if ci == N_COMP - 1:
...
fig.text(0.5, (fig_h - 0.18) / fig_h,
         'Chromatin & Soluble Nuclear — union of significant proteins '
         '(corrected p > 3.3, |FC| > 0.5 in ≥1 condition vs Naïve)',
         ha='center', va='center', fontsize=10,
         fontfamily='Arial', color='#444444', style='italic')
