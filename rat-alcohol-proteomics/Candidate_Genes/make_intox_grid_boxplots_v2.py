"""
Intox-representative box-and-whisker plots — 2×2 grid
======================================================
Top-left:     Cat   (Membrane)
Top-right:    Eno1  (Soluble Nuclear)
Bottom-left:  Acss2 (Chromatin)
Bottom-right: Adh5  (Cytosol)

All 4 conditions shown. Y-axis label on left column only.
AW-M-3 excluded. Asterisks where corrected p > 3.3.

Outputs:
  Intox_Grid_BoxPlots.pdf / .png
"""
...
OUT_PDF    = os.path.join(SCRIPT_DIR, 'Intox_Grid_BoxPlots_v2.pdf')
OUT_PNG    = os.path.join(SCRIPT_DIR, 'Intox_Grid_BoxPlots_v2.png')
...
PANELS = [
    ...
    dict(gene='Acss2', sheet='Chromatin',       prefix='Chrom',title='Acss2\n(Chromatin)'),
...
for idx, (ax, cfg) in enumerate(zip(ax_flat, PANELS)):
    ...
    # Y-axis label on left column only (idx 0 and 2)
    if idx in (0, 2):
