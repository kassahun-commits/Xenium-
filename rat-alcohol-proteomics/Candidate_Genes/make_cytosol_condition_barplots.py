"""
Cytosol fraction bar/dot plots — one panel per condition
=========================================================
Panel 1 (Intox): Gclm  — Naive vs Intoxication
Panel 2 (Intox): Adh5  — Naive vs Intoxication
Panel 3 (AW):    Apip  — Naive vs Acute Withdrawal
Panel 4 (PA):    Txn2  — Naive vs Protracted Abstinence

Same purple pastel style as make_rep_dotplots.py.
AW-M-3 excluded from AW replicate columns.

Outputs:
  Cytosol_Condition_BarPlots.pdf / .png
"""
...
OUT_PDF    = os.path.join(SCRIPT_DIR, 'Cytosol_Condition_BarPlots.pdf')
OUT_PNG    = os.path.join(SCRIPT_DIR, 'Cytosol_Condition_BarPlots.png')

SHEET  = 'Cytosol'
PREFIX = 'Cyto'

PANELS = [
    dict(gene='Gclm', cond='Intox', cond_prefix='I',
         corr_col='Corrected',
         xlabel='Intox',
         title='Gclm\n(Cytosol — Intox)'),
    dict(gene='Adh5', cond='Intox', cond_prefix='I',
         corr_col='Corrected',
         xlabel='Intox',
         title='Adh5\n(Cytosol — Intox)'),
    dict(gene='Apip', cond='AW',    cond_prefix='AW',
         corr_col='Corrected.1',
         xlabel='Acute\nWithdrawal',
         title='Apip\n(Cytosol — AW)'),
    dict(gene='Txn2', cond='PA',    cond_prefix='PA',
