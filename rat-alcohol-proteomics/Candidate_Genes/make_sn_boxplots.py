"""
Soluble Nuclear fraction box-and-whisker plots — 3 panels, all 4 conditions
============================================================================
Panel 1: Eno1    — Naïve / Intox / AW / PA  (Intox representative)
Panel 2: Gapdh   — Naïve / Intox / AW / PA  (AW representative)
Panel 3: Mthfd1  — Naïve / Intox / AW / PA  (PA representative)
...
OUT_PDF    = os.path.join(SCRIPT_DIR, 'SN_BoxPlots.pdf')
OUT_PNG    = os.path.join(SCRIPT_DIR, 'SN_BoxPlots.png')

SHEET  = 'Soluble nuclear'
PREFIX = 'Nuc'

PANELS = [
    dict(gene='Eno1',   title='Eno1\n(Soluble Nuclear)'),
    dict(gene='Gapdh',  title='Gapdh\n(Soluble Nuclear)'),
    dict(gene='Mthfd1', title='Mthfd1\n(Soluble Nuclear)'),
]
...
np.random.seed(42)
df = pd.read_excel(DATA_FILE, sheet_name='Soluble nuclear')
