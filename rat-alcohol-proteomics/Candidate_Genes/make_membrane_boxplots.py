"""
Membrane fraction box-and-whisker plots — 3 panels, all 4 conditions
=====================================================================
Panel 1: Cat    — Naïve / Intox / AW / PA  (Intox representative)
Panel 2: Sirt3  — Naïve / Intox / AW / PA  (AW representative)
Panel 3: Dmgdh  — Naïve / Intox / AW / PA  (PA representative)

Asterisks shown only for conditions with corrected p > 3.3 vs Naïve.
AW-M-3 excluded from AW replicates.

Outputs:
  Membrane_BoxPlots.pdf / .png
"""
...
OUT_PDF    = os.path.join(SCRIPT_DIR, 'Membrane_BoxPlots.pdf')
OUT_PNG    = os.path.join(SCRIPT_DIR, 'Membrane_BoxPlots.png')

SHEET  = 'Membrane'
PREFIX = 'Memb'

PANELS = [
    dict(gene='Cat',   title='Cat / Catalase\n(Membrane)'),
    dict(gene='Sirt3', title='Sirt3\n(Membrane)'),
    dict(gene='Dmgdh', title='Dmgdh\n(Membrane)'),
]
...
for ax, cfg in zip(axes, PANELS):
    ...
    # Significance asterisks above each non-Naive condition
