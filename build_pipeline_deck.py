...
def main():
    ...
    ap.add_argument('--combined-dotplot-pdf', default=None,
    ...
    combined_dot_png = None
    if args.combined_dotplot_pdf and Path(args.combined_dotplot_pdf).exists():
    ...
    # ---- Slide 7d: combined Neuron vs Astrocyte dot plot, both iterations side by side ----
    if combined_dot_png is not None:
        s = add_blank(prs)
        slide_header(
            s, prs, 'Curated gene panel — whole-cell vs nucleus-only, side by side',
            'Same 42 genes, same axes · left half = whole-cell, right half = '
            'nucleus-only (both marker-gene typed, Wilcoxon)',
            tag='Neuron vs Astrocyte · iteration compare', tag_color=SLATE)
        # large landscape figure; size by HEIGHT so it never overflows the
        # slide, then centre horizontally. aspect ~1.97 (14.0 x 7.1)
        from PIL import Image as _PILImage
        _iw, _ih = _PILImage.open(str(combined_dot_png)).size
        pic_h = 5.92
        pic_w = pic_h * (_iw / _ih)
        pic_left = (prs.slide_width / 914400 - pic_w) / 2.0
        s.shapes.add_picture(str(combined_dot_png),
